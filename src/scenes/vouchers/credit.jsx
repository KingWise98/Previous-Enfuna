...
const CreditVoucher = () => {
  ...
  // Fetch credit vouchers from API
  useEffect(() => {
    const fetchVouchers = async () => {
      try {
        setIsLoading(true);
        // TODO: Replace with actual API endpoint
        // const response = await fetch('/api/credit-vouchers');
        ...
        console.error('Error fetching credit vouchers:', error);
        setIsLoading(false);
      }
    };
  ...
  const handleCreateVoucher = async () => {
    try {
      ...
      */
      ...
      console.error('Error creating credit voucher:', error);
    }
  };
  ...
  const handleDeleteVoucher = async (id) => {
    try {
      ...
      // await fetch(`/api/credit-vouchers/${id}`, { method: 'DELETE' });
      setVouchers(vouchers.filter(voucher => voucher.id !== id));
      setOpenDeleteDialog(false);
    } catch (error) {
      console.error('Error deleting credit voucher:', error);
    }
  };
  ...
  return (
    <Box p={3}>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="subtitle1" color="textSecondary">
            Record and track all credit transactions
          </Typography>
        </Box>
        <Box>
          <Button
                  ...
                  <TableRow>
                    <TableCell colSpan={8} align="center" sx={{ py: 4 }}>
                      <Typography variant="subtitle1">
                        {vouchers.length === 0 ? 'No credit vouchers found' : 'No matching vouchers found'}
                      </Typography>
                    </TableCell>
                  </TableRow>
                )}
      ...
      >
        <DialogTitle sx={{ borderBottom: `1px solid ${theme.palette.divider}` }}>
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Typography variant="h6">New Credit Voucher</Typography>
      ...
      >
        ...
        <DialogContent dividers>
          <Typography>
            Are you sure you want to delete the credit voucher for {voucherToDelete?.accountName}?
          </Typography>
